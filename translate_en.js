...
// --- 1. Generate en/diary/page.js ---
const enDiaryContent = `import ShareWidget from '@/components/ShareWidget';
...
fs.mkdirSync('app/en/diary', { recursive: true });
fs.writeFileSync('app/en/diary/page.js', enDiaryContent);
console.log('Created en diary page.');

// --- 2. Update en/page.js ---
let enPage = fs.readFileSync('app/en/page.js', 'utf8');

// Ensure ShareWidget is imported
if (!enPage.includes('ShareWidget')) {
  enPage = "import ShareWidget from '@/components/ShareWidget';\n" + enPage;
}

// Remove todaysMessage definition
enPage = enPage.replace(/const todaysMessage = \[\s*\{[\s\S]*?\}\s*\];/g, '');

// Replace Hero/Video and Message Section
// We will search for {/* Video Section */} and everything before {/* Detailed Business Section */}
// Wait, en/page.js currently just has {/* Video Section */} then {/* Today's Message Section */} then {/* Detailed Business Section */}
// Let's replace the whole chunk from <div className="home-wrapper"> up to {/* Detailed Business Section */}
const wrapperStart = enPage.indexOf('<div className="home-wrapper">');
const businessStart = enPage.indexOf('{/* Detailed Business Section */}');

const newHeroAndVideo = `<div className="home-wrapper">
      {/* Hero Section */}
...
enPage = enPage.substring(0, wrapperStart) + newHeroAndVideo + enPage.substring(businessStart);

// Inject ShareWidgets into Business, Charity, Ideology in en/page.js
enPage = enPage.replace(/<\/div>\s*<\/div>\s*<\/section>\s*\{\/\* Detailed Philanthropy/g,
  `</div>
...
enPage = enPage.replace(/<\/div>\s*<\/div>\s*<\/section>\s*\{\/\* Ideology Section/g,
  `</div>
          <div style={{ display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
...
enPage = enPage.replace(/<span className="quote-author">Nusrat Fatima Naqvi<\/span>\s*<\/div>\s*<\/div>\s*<\/section>/g,
...
fs.writeFileSync('app/en/page.js', enPage);
console.log('Updated en page.');
